...
static func register_tag(tag: Callable, priority: int = 0) -> void:
	...
	for prio_group in prio_groups:
		_ordered_tags.append_array(_tags[prio_group])
...
static func _make_setting(setting_name: String) -> Dictionary:
	return {
		"name": setting_name,
...
func _init(p_module: String, p_name: String) -> void:
	module = p_module
	name = p_name


func trace(text: String, values: Array = []) -> void:
...
func info(text: String, values: Array = []) -> void:
	_log_text(text, values, LOG_INFO)


func warning(text: String, values: Array = []) -> void:
...
func error(text: String, values: Array = []) -> void:
...
func _log_text(text: String, values: Array, level: int) -> void:
