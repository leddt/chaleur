...
static func put_speed_in_hand(p: PlayerState, values: Array[int]) -> Array[String]:
	...
	for i in values.size():
		var card := HeatCard.new("%s_test_spd_%d" % [p.display_name, i], HeatCard.Kind.SPEED, values[i])
		p.hand.add(card)
		ids.append(card.id)
	return ids
...
static func ensure_engine_heat(p: PlayerState, amount: int) -> void:
	p.engine.clear()
	for i in amount:
		p.engine.add(HeatCard.new("%s_h_%d" % [p.display_name, i], HeatCard.Kind.HEAT, 0))
...
static func play_speeds(engine: HeatGameEngine, player_id: int, values: Array[int]) -> bool:
	...
	while p.hand.size() < 7:
		p.hand.add(HeatCard.new("%s_pad_%d" % [p.display_name, p.hand.size()], HeatCard.Kind.SPEED, 1))
